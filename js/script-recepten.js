    ...
    .then(function (response) {
        ...
        for (let i = 0; i < response.length; i++) {
            ...
            <h2 class="accordion-header">
              ...
              <img src="${recipe.image}" class="img-small float-start me-2" alt="${recipe.title}" width="150"><span class="h6 mt-2">ingrediënten</span>: ${recipe.ingredients} <hr><span class="h6  mt-2">bereiding</span>: ${recipe.description}</div>
            </div>
      </div>`;
        }
        html += '</div>';
